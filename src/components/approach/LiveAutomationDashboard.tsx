...
const LiveAutomationDashboard = () => {
  // Base realistic numbers (mid-market global automation activity)
  const [stats, setStats] = useState({
    companiesDeployingPerHour: 47,
    totalDeployed: 1240,
    successfullyDeployed: 782,
    aiPowered: 591,
    blockedByData: 298,
    failedOrNonPerforming: 160,
  });

  // Live update logic (every 12 seconds)
  useEffect(() => {
    const interval = setInterval(() => {
      setStats(prev => {
        // Simulate new activity
        const newDeployments = Math.floor(Math.random() * 5) + 2; // 2-6 new deployments
        const newSuccess = Math.floor(newDeployments * 0.63); // ~63% success rate
        const newAI = Math.floor(newDeployments * 0.48);
        const newBlocked = Math.floor(newDeployments * 0.24);
        const newFailed = newDeployments - newSuccess;

        return {
          companiesDeployingPerHour: Math.max(38, Math.min(62, prev.companiesDeployingPerHour + (Math.random() > 0.5 ? 1 : -1))),
  ...
  const options = {
    ...
    scales: {
      ...
      y: { 
        grid: { display: false }, 
        ticks: { color: '#F5F5F5', font: { size: 12, weight: 'bold' as const } } 
      },
    },
  };
    ...
    <div className="bg-[#111] border border-white/10 rounded-[32px] p-8 md:p-10 shadow-2xl">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-10 gap-6">
        ...
        <div className="text-left md:text-right bg-white/5 border border-white/10 px-6 py-4 rounded-2xl">
          <div className="text-3xl font-bold text-emerald-500 tabular-nums">{stats.companiesDeployingPerHour}</div>
      ...
      <p className="text-[10px] text-[#444] uppercase tracking-[0.2em] font-bold mt-10 text-center">
        Numbers update live to simulate real-world activity. Data issues remain the #1 blocker for successful autonomy.
      </p>
    </div>
  );
};
